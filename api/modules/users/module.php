...
function users_loggedIn(){
	...
	if(is_object($user)){
    	...
    	if($user[0]->idUser == 0 && conf_get('embeded', 'core', false) == false){
        	grace_debug("User id = 0, this can't be logged in");
			return false;
		}
...
function users_registerNew(){
	...
	if(is_object($newUserByName) && is_object($newUserByEmail) && $newUserByName->idUser == 0 && $newUserByEmail->idUser == 0){
...
function users_generateSessionKey($idUser){
    ...
    $sessionKey = md5(time() * rand(0, 1000));
...
function users_updateLastAccess(){
	...
	if(params_get('sessionKey', '') != '' && $idUser > 0){
...
function _users_update($dets){
	...
	$q = sprintf("UPDATE users SET `fullName` = '%s',
		...
		`avatar` = '%s'
		WHERE `idUser` = %s",
		addslashes($newDets['fullName']),
		users_cleanName($newDets['userName']),
		$newDets['email'],
		addslashes($newDets['about']),
		$newDets['country'],
		$newDets['status'],
		$newDets['timestamp'],
		$newDets['lastAccess'],
...
function users_recoverPwd(){
	...
	$user->pwd = substr(md5(rand(0, 1000) + time()), 0, 6);
	...
	if(_users_update((array)$user) == SUCCESS_ALL_GOOD){
		...
		$resp = mailer_sendEmail(array(
			'to' => $user->email,
			'subject' => 'Recuperaci¨®n de Clave ' . conf_get('siteName', 'core', 'Mi Sitio'),
